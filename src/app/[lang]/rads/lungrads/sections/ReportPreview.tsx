...
import { generateLungReportText } from "../report/reportTextGenerator";
import { trackRadsReportCopied } from "@/lib/analytics";
import CollapsibleSection from "./CollapsibleSection";

export default function ReportPreview() {
  const { state, scoredNodules } = useLungReport();
  const [copied, setCopied] = useState(false);

  const reportText = useMemo(
    () => generateLungReportText(state, scoredNodules),
    [state, scoredNodules],
  );

  const handleCopy = async () => {
    await navigator.clipboard.writeText(reportText);
    trackRadsReportCopied("lungrads");
    setCopied(true);
