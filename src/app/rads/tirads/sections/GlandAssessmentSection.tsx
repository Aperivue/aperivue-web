  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-1">
        <input type="number" step="0.1" min="0" placeholder="L" value={value.l}
        ...
        <input type="number" step="0.1" min="0" placeholder="W" value={value.w}
          onChange={(e) => onChange({ ...value, w: e.target.value })}
          className="w-20 rounded-lg border border-border bg-surface px-2 py-1.5 text-sm text-center" />
        <span className="text-xs text-foreground/40">&times;</span>
        <input type="number" step="0.1" min="0" placeholder="H" value={value.h}
  ...
  return (
    <CollapsibleSection title="Thyroid Gland" defaultOpen={false}>
      <div className="space-y-4">
        ...
        <div className="w-40">
          <label className="mb-1 block text-xs font-medium text-foreground/60">Isthmus AP (cm)</label>
          <input type="number" step="0.1" min="0" placeholder="e.g. 0.3"
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          <div>
            <label className="mb-1 block text-xs font-medium text-foreground/60">Echotexture</label>
            <select value={g.echotexture} onChange={(e) => set({ echotexture: e.target.value as typeof g.echotexture })}
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-foreground/60">Vascularity</label>
            <select value={g.vascularity} onChange={(e) => set({ vascularity: e.target.value as typeof g.vascularity })}
              className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm">
              <option value="">--</option>
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-foreground/60">Thyroiditis</label>
            <select value={g.thyroiditis} onChange={(e) => set({ thyroiditis: e.target.value as typeof g.thyroiditis })}
              className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm">
              <option value="">--</option>
        ...
        <div>
          <label className="mb-1 block text-xs font-medium text-foreground/60">Additional gland findings</label>
          <textarea
            rows={2}
